...
javacvbedrock/
  ...
  loading/       Pack source abstraction, resource discovery, validation, lazy content cache, and ResourceIndex.
...
The loading pipeline accepts local folders and ZIP-compatible archives (`.zip`, `.mcpack`, `.mcaddon`). It normalizes them behind a `PackSource` abstraction so future parsers do not need to know whether resources came from disk, an archive, remote storage, or cloud storage. Discovery returns only a `ResourceIndex`; it does not parse, convert, generate, or package resources.

Every indexed file records its namespace, relative path, source URI, resource type, extension, size, SHA-256 checksum, last modified time, and origin pack. Detection is metadata and layout based: `pack.mcmeta` plus `assets/` identifies vanilla-style packs, while known plugin metadata/layout markers identify ItemsAdder, Nexo, and Oraxen candidates without depending only on folder names. Unknown folders and unknown file types are retained in the index and reported as diagnostics instead of being discarded.
